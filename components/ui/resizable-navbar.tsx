...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        'relative z-[60] mx-auto hidden w-full max-w-5xl flex-row items-center justify-between self-start rounded-full bg-white px-4 py-2 dark:bg-neutral-950 md:flex',
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-rose-600/10 dark:bg-rose-600/10"
            />
          )}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <X className="text-rose-600 dark:text-white" onClick={onClick} />
  ) : (
    <Menu className="text-rose-600 dark:text-white" onClick={onClick} />
  );
};

export const NavbarLogo = ({ width, height }: { width: number, height: number }) => {
  return (
    <Link href="/" className="z-50 flex items-center justify-center gap-2 p-2 md:p-3 transition-all hover:opacity-90">
      <Image src="/assets/kinder_logo.svg" alt="logo" height={height} width={width} draggable={false} />
    </Link>
  );
};
